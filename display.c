...
#define FOREGROUNDCOLOR GRAPHICS_COLOR_YELLOW
...
void DrawBoardGame(tcellstate map[9], uint32_t color1, uint32_t color2)
{
    ...
    Graphics_setForegroundColor(&g_sContext, color1);
    ...
    // Draw standard X O
    Graphics_setForegroundColor(&g_sContext, color2);
...
void PrintStringEC(char *str, int row, int col) {
    Graphics_setBackgroundColor(&g_sContext, FOREGROUNDCOLOR);
    int i;
    for (i = 0; str[i] != '\0'; i++) {
        LCDDrawChar(row,  col, str[i]);
        col++;
